  ...
  const calculateLoan = () => {
    if (principal > 0 && interest > 0 ) {
      ...
      let calcemi =
        p *
        actualRate *
        (Math.pow(1 + actualRate,1) / (Math.pow(1 + actualRate,1) - 1));

      setEmi(Math.round(calcemi));
      setAmount(Math.round(calcemi * 1));
      setTotalinterest(Math.round(calcemi * 1 - p));
    } else {
        ...
        <div className="grid md:grid-cols-2 gap-10">
          <form className="p-8 bg-white/10 z-10 backdrop-filter backdrop-blur-lg shadow-xl rounded-md ring-1 ring-gray-50">
            ...
            <div className="py-5">
              ...
              <select
                ...
                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                <option defaultValue value={2.89}>pay for goods or services</option>
          ...
          <div className="right space-y-2">
            ...
            <div className="bg-gray-50 rounded-lg shadow-md p-5">
              Paypal fees: <span className="ml-10">$ {totalinterest}</span>
            </div>
            <div className="bg-gray-50 rounded-lg shadow-md p-5">
